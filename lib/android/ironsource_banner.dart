...
abstract class IronSourceBannerListener {
  Future<Null> _handle(MethodCall call) async {
    if (call.method == BANNER_CLICKED)
      onBannerAdClicked();
    else if (call.method == BANNER_LEFT_APPLICATION)
      onBannerAdLeftApplication();
    else if (call.method == BANNER_LOAD_FAILED)
      onBannerAdLoadFailed(call.arguments);
    else if (call.method == BANNER_LOADED)
      onBannerAdLoaded();
    else if (call.method == BANNER_SCREEN_DISMISSED)
      onBannerAdScreenDismissed();
    else if (call.method == BANNER_SCREEN_PRESENTED)
